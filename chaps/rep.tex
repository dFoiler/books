...
\begin{proof}
	...
	\begin{itemize}
		...
		Of course, scalars in $\CC$ are morphisms, and these are distinct morphisms because $\rho$ is irreducible and hence nonzero. However, \Cref{thm:schur} tells us that these are the only morphisms, so $\dim\op{End}_{\CC[G]}(V_\rho)=\dim\CC=1$.
...
\end{proposition}
\begin{proof}
	This is tricky. That these characters are orthonormal follows from \Cref{cor:first-ortho}, so it remains to show that these span the vector space of class functions. Let $\rho_1,\ldots,\rho_k$ be the irreducible $G$-representaitons. Now, for any class function $\psi$, we define
...
\end{corollary}
\begin{proof}
	Let the conjugacy classes of $G$ be represented as $[g_1],\ldots,[g_r]$; note that this is equal to the number of irreducible representations by \Cref{cor:num-irreps}. The point here is to do linear algebra to achieve the result from \Cref{cor:first-ortho}. Indeed, define the $r\times r$ matrix
